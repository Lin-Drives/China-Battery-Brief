...
| **JWT** | 一种「盖章的通行证」字符串，服务器验证签名即认账，本身不存服务器端 | `api/lib/jwt.ts` |
...
**特别提示**：防护按攻击行为和来源信誉执行，不按地域预设。重点处理自动扫描、伪造 XFF、爆破和异常频率；对应措施是**隐藏文件拦截**（路径探测直接 404）、**限流**（频率控制）和 **Origin 校验**（拒绝假来源）。
...
```
- 保留份数：本地默认由 `BACKUP_RETENTION` 控制（默认 14 份）；生产 VPS 的日常备份按 `docs/deploy.md` 保留最近 7 份，两者不要混淆
...
2. **隔离**：先在 CDN/网关封禁可疑 IP 段（快速止血）；必要时在 VPS 停服（`systemctl stop cbb`）或在平台侧停容器。
...
- **支付与邮件的边界不同**：支付 checkout 仍为模拟；邮件已支持 SMTP、双重确认和群发，但未配置 SMTP 时只记录日志而不发送。接入真实支付后，需补「第三方凭证管理」和「回调验签」两节。
